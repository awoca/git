...
final class TreeTests: Tests {
    func testEmpty() {
        XCTAssertEqual("4b825dc642cb6eb9a060e54bf8d69288fbee4904", Tree.save(url, index: []).hash)
    ...
    func testNewRepository() {
        ...
        git.create(url).sink(receiveCompletion: { _ in }) { _ in
            XCTAssertEqual("4b825dc642cb6eb9a060e54bf8d69288fbee4904", Tree.save(self.url, index: []).hash)
    ...
    func testNewRepositoryOneFile() {
        ...
        try! Data("hello world".utf8).write(to: url.appendingPathComponent("myfile.txt"))
        git.create(url).sink(receiveCompletion: { _ in }) { _ in
            XCTAssertEqual("007a8ffce38213667b95957dc505ef30dac0248d", Tree.save(self.url, index: []).hash)
